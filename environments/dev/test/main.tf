...
output "validation_command" {
  value = "az vm run-command invoke --resource-group ${data.terraform_remote_state.dev_workload.outputs.workload_rg_name} --name vm-test-dev --command-id RunShellScript --scripts 'cat /var/log/validation.log' --subscription 6f5ab5c9-4f1c-43c2-b269-89441976bb0a"
}
